...
def _parse_graphql_response(response):
    """Parses Facebook's GraphQL API response to check for success or specific errors."""
    try:
        response_json = response.json()
        if 'errors' in response_json:
            error_msg = str(response_json['errors']).lower()
            if 'oauth' in error_msg or 'login required' in error_msg or 'viewer does not have permission' in error_msg:
                return {'status': 'cookie_dead', 'message': 'Lỗi xác thực, cookie có thể đã hết hạn.'}
            if 'temporarily blocked' in error_msg or 'spam' in error_msg:
                 return {'status': 'action_failed', 'message': 'Hành động bị chặn do spam.'}
            return {'status': 'action_failed', 'message': f"Lỗi không xác định từ FB: {error_msg[:100]}"}
        
        if 'data' in response_json and response_json.get('data'):
            return {'status': 'success', 'data': response_json['data']}

        return {'status': 'action_failed', 'message': 'Phản hồi không chứa dữ liệu hoặc lỗi.'}
    except json.JSONDecodeError:
        return {'status': 'action_failed', 'message': 'Không thể giải mã phản hồi JSON từ Facebook.'}
    except Exception as e:
        return {'status': 'action_failed', 'message': f'Lỗi không mong muốn khi phân tích phản hồi: {e}'}
...
def comment_fb(data, object_id, msg, proxy=None):
    ...
    try:
        ...
        if parsed_result['status'] == 'success':
            try:
                comment_text = parsed_result['data']['comment_create']['feedback_comment_edge']['node']['preferred_body']['text']
                prints(5, 255, 0, f'Đã comment "{comment_text}"', end='\r')
                if comment_text == msg:
                    parsed_result['payload'] = comment_text
                    return parsed_result
                else:
                    return {'status': 'action_failed', 'message': 'Nội dung comment trả về không khớp.'}
            except (KeyError, TypeError):
                return {'status': 'action_failed', 'message': 'Cấu trúc phản hồi comment không mong muốn.'}
        return parsed_result
    except requests.exceptions.RequestException as e:
        return {'status': 'action_failed', 'message': f'Lỗi kết nối khi comment: {e}'}
...
def lam_job(data, jobs, type_job_doing, current_proxy=None):
    ...
    # Mặc định kết quả
    result = {'status': 'action_failed', 'verification_failed': False, 'payload': None, 'message': 'Unknown action'}
    ...
    if type_job_doing == 'review_facebook':
        ...
        if res_get_post_id.get('page_id'):
            ...
            return dexuat_result # Trả về trực tiếp dict kết quả
        else:
        ...
        if post_id_to_comment:
            comment_result = comment_fb(data, post_id_to_comment, jobs['data'], current_proxy)
            if comment_result['status'] == 'success':
                comment_text = comment_result['payload']
                ...
                if is_verified:
                    comment_result['payload'] = True
                    return comment_result
                else:
                    result['status'] = 'verification_failed'
                    result['message'] = 'Bình luận không được tìm thấy sau khi đăng.'
            else:
                return comment_result
        else:
...
Nhập 1 để bắt đầu chạy tool: ''',end='')
    
    x=input()
    job_map = {'1': 'like_facebook', }
    for i in x.split(','):
        job_type = job_map.get(i.strip())
        if job_type:
            ...
            for type_job in list_type_job:
                ...
                for job in jobs_for_current_type:
                    ...
                    if (tasks_on_current_cookie >= COOKIE_JOB_LIMIT or consecutive_failures >= CONSECUTIVE_FAILURE_LIMIT):
                        break # Break job loop to trigger account switch
                    ...
                    try:
                        ...
                        if job_result['status'] == 'success':
                            ...
                            if res_submit[0]:
                                ...
                                save_job_history(job_history) # Save after each success
                            ...
                            demsk += 1
                            consecutive_failures = CONSECUTIVE_FAILURE_LIMIT # Force switch
                            break # Break from job loop to switch
                        else: # action_failed or verification_failed
                ...
                if (tasks_on_current_cookie >= COOKIE_JOB_LIMIT or consecutive_failures >= CONSECUTIVE_FAILURE_LIMIT):
                    break # Break from job type loop
